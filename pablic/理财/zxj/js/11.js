...
(function (c) {
  c.fn.extend({
    actual: function (d, m) {
      ...
      var a = c.extend({absolute: !1, clone: !1, includeMargin: !1}, m), g = this.eq(0), h, b;
      if (!0 === a.clone) h = function () {
        g = g.clone().attr("style", "position: absolute !important; top: -1000 !important; ").appendTo("body")
      }, b = function () {
        g.remove()
      }; else {
        var k = [], l = "", e;
        h = function () {
          e = g.parents().addBack().filter(":hidden");
          l += "visibility: hidden !important; display: block !important; ";
          !0 ===
          a.absolute && (l += "position: absolute !important; ");
          e.each(function () {
            var a = c(this), b = a.attr("style");
            k.push(b);
            a.attr("style", b ? b + ";" + l : l)
          })
        };
        b = function () {
          e.each(function (a) {
            var b = c(this);
            a = k[a];
            void 0 === a ? b.removeAttr("style") : b.attr("style", a)
          })
        }
      }
      h();
      h = /(outer)/.test(d) ? g[d](a.includeMargin) : g[d]();
      b();
      return h
    },
    smartShow: function (opts) {
      ...
      return this.each(function (key, obj) {
        var node = $(options.panel), fn = {}, temp, getData = null, showList = null, autoShow = false, isChrome = navigator.userAgent.indexOf('Chrome'), index = -1;
        ...
        getData = function (value) {
          $.ajax({
            type: 'get',
            url: options.url,
            data: options.param + '=' + encodeURIComponent(value),
            ...
            success: function (data) {
              if (data) {
                ...
                autoShow = true;
                index = -1;
                var nodes = $(options.panel).find(options.dataWrap);
                if (nodes.length) {
                  nodes.bind({
                    mouseover: function () {
                      index = $(this).index();
        ...
        $(obj).bind({
          keydown: function () {
            ...
            var item = node.find(options.dataWrap), keyCode = event.keyCode, value = $.trim($(obj).val()), fn = {}, length = item.length;
            fn = {
              moveTo: function (k) {
                if (k < 0) {
                  index = k = length - 1
    ...
    tab: function (d) {
      var m = null, a = c.extend(!0, {
        ...
        defaultPanel: 0,
        async: {
          state: !1, url: "a.php", type: "get", dataType: "json",
          data: [], error: function () {
          }, success: function (a, c) {
          }
        },
        slideTab: {trigger: ".factor", sliding: !1, vertical: !1, speed: 300},
        opacity: !1,
        delay: 200,
        operate: function () {
        }
      }, d);
      a.delay = "click" === a.type ? 0 : a.delay;
      m = function (a) {
        for (var c = a.length, b = 0, k = {}; b < c; b++)k[a[b].split("=")[0]] = a[b].split("=")[1];
        return k
      };
      return this.each(function (d, h) {
        var b = c(h).find(a.tab), k = c(h).find("." + a.panelClass), l, e = "left", p = "width", q = {}, t = null, r = null, n = null;
        l = !0 === a.opacity ? 600 : 0;
        !0 === a.slideTab.sliding ? !0 === a.slideTab.vertical ? (e =
              "top", p = "height", c(a.slideTab.trigger).height(b.eq(a.defaultPanel).actual("outerHeight"))) : c(a.slideTab.trigger).width(b.eq(a.defaultPanel).actual("outerWidth")) : 0;
        t = function (a) {
          q[e] = b.eq(a).position()[e];
          q[p] = "width" === p ? b.eq(a).actual("outerWidth") : b.eq(a).actual("outerHeight");
          return q
        };
        r = function (f) {
          var d = a.async.data[f], e = "", g = {};
          b.eq(a.defaultPanel).attr("data-j-tab", 1);
          b.eq(f).addClass(a.triggerClass).siblings().removeClass(a.triggerClass);
          c(h).find(a.slideTab.trigger).animate(t(f), a.slideTab.speed);
          !0 === a.async.state && "1" !== b.eq(f).attr("data-j-tab") && 0 !== d && (e = 1 === a.async.url.split(",").length ? a.async.url.split(",")[0] : a.async.url.split(",")[f], g = -1 < d.indexOf("&") ? m(d.split("&")) : m(d.split()), c.ajax({
            ...
            error: function () {
              a.async.error(k.eq(f))
            },
            success: function (c) {
              b.eq(f).attr("data-j-tab", 1);
              a.async.success(k.eq(f), c)
            }
          }));
...
var control = {
  ...
  topDis: function () {
    if ($('.table-box').length != 0) {
      return $('.table-box').offset().top
    } else {
      return 0
    }
  },
  inputVal: function () {
    function inputDef(obj, text) {
      obj.on({
        focus: function () {
          var val = $.trim(obj.val());
          if ($.trim(obj.val()) == text) {
            obj.val('').css('color', '#34444d')
          }
        }, blur: function () {
          var val = $.trim(obj.val());
          if (val == '') {
            obj.val(text).css('color', '#aeb8c6')
          } else {
            obj.val(val)
          }
        }
    ...
    inputDef($('.header-inbox'), '请输入基金代码/名称/简拼/组合名称/要找的功能')
  },
